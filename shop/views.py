...
def category(request, id):
    category_filter = Product.objects.filter(
        categories=id)  # экземпляр класса из моделей   Вариант: board=Tovar.objects.all().  return render(request, "boards.html", {'ky': board})
...
def product(request, itm_id):
    product = Product.objects.get(
        id=itm_id)  # нов пер=из класса моделей получаем из бд объекты по url itm_id (передаем вторым параметром в скобках функции)!
    if request.method == 'POST':  # кнопка добавить в корзину
...
def post(request, product):
    ...
    user, _ = UserBasket.objects.get_or_create(user_kay=request.session.session_key)  #если существует и запиши в юзер. если не существестует создай в _
    ...
    if get_basket:
        ...
        if validate(product, get_basket, request):                # Недостаточное количество товара на складе!
            get_basket.save()
    else:
        if validate(product, get_basket, request):              
...
def validate(product, get_basket, request):
    valid = True
    if get_basket:
        if product.quantity < get_basket.quantity:
